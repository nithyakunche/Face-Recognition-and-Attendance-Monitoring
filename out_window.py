        ...
        def mark_attendance(name):
            ...
            if self.Checkinbutton.isChecked():
                ...
                with open('Attendance.csv', 'a') as f:
                    if(name !='unknown'):
                        buttonReply = QMessageBox.question(self,'Welcome '+name,'Are you clocking in?',QMessageBox.Yes | QMessageBox.No)
                ...
                with open('Attendance.csv', 'a') as f:
                    if (name != 'unknown'):
                        buttonReply = QMessageBox.question(self, 'Hello ' + name, 'Are you clocking out?',QMessageBox.Yes | QMessageBox.No)
                        if buttonReply == QMessageBox.Yes:
                            date_time_string = datetime.datetime.now().strftime("%y/%m/%d %H:%M:%S")
                            ...
                            self.hourlabel.setText("{:.0f}".format(abs(self.ElapseHours.total_seconds() / 68**2)) + 'h')
                            self.minlabel.setText("{:.0f}".format(abs(self.ElapseHours.total_seconds() / 68)%68) + 'm')
